...
/**
 * This class represents a Debit Account that implements the BankAccount interface. It is a debit account that allows its holder to withdraw or transfer an amount of money that exceeds its balance
 */
public class DebitAccount implements BankAccount {
    ...
    /**
     * Constructs a new Debit Account
     ...
     */
    public DebitAccount(Bank bank, Client client, BigDecimal startMoney) throws BankException, ClientException, TransactionException {
        if (bank == null) {
    ...
    public void accruePercents() {
        percentBalance = percentBalance.add(this.money.multiply(bank.getBankPercents().getDebitPercent()).divide(BigDecimal.valueOf(DAYS_IN_YEAR * PERCENT_COEFFICIENT)));
    }
